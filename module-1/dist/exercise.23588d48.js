...
async function drawLineChart() {
  ...
  const wrapper = d3.select("#exercise").append("svg").attr("width", dimensions.width).attr("height", dimensions.height);
  ...
  // draw data
  const avgNumOfColorsOverallPlacement = yScale(data.map(d => d.avgNumOfColors).reduce((acc, val) => acc + val, 0) / data.length);
  const avgNumOfColorsOverall = bounds.append("rect").attr("x", 0).attr("width", dimensions.boundedWidth).attr("y", avgNumOfColorsOverallPlacement).attr("height", dimensions.boundedHeight - avgNumOfColorsOverallPlacement).attr("fill", "#e0f3f3");
  const lineGenerator = d3.line().x(d => xScale(xAccessor(d))).y(d => yScale(yAccessor(d)));
  const line = bounds.append("path").attr("d", lineGenerator(data)).attr("fill", "none").attr("stroke", "black").attr("stroke-width", 2);
...
},{}]},{},["node_modules/parcel-bundler/src/builtins/hmr-runtime.js","exercise.js"], null)
//# sourceMappingURL=/exercise.23588d48.js.map